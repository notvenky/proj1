...
class MyActor(nn.Module):
    def __init__(self, num_actuators, frequencies=None, amplitudes=None, phases=None, init_std=0.5):
        ...
        self.offset = nn.Sequential(
            nn.Linear(qpos + qvel + 1, 64),
            nn.ReLU(inplace=True), nn.Linear(64, 64), nn.ReLU(inplace=True),
            nn.Linear(64, 64), nn.ReLU(inplace=True), nn.Linear(64, 64),
            nn.ReLU(inplace=True), nn.Linear(64, 64), nn.ReLU(inplace=True),
            num_actuators
        )
