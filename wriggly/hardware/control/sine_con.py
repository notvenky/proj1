    ...
    while True:
        for dxl_id in DXL_ID_LIST:
            if dxl_id in [11, 20, 22]:  # Assuming these are the equivalent of [0, 2, 4] in the simulation
                ...
                goal_positions[dxl_id] = 1.57 * np.sin(2*np.pi * freq_even * time.time() + phase)
            elif dxl_id in [12, 21]:  # Assuming these are the equivalent of [1, 3] in the simulation
                phase = -np.pi/2 if dxl_id == 12 else np.pi/2
                goal_positions[dxl_id] = 3.14 * np.sin(2*np.pi * freq_odd * time.time() + phase)
            ...
            # Read present position for all Dynamixels
            dxl_present_position, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, dxl_id, ADDR_PRO_PRESENT_POSITION)
            if dxl_comm_result == COMM_SUCCESS:
                current_positions[dxl_id] = dxl_present_position

            # Add a sleep time to simulate the control loop
            time.sleep(1.0 / 60.0)
